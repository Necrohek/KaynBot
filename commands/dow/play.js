  ...
  download: async (url, format) => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        throw new Error('Fallo al obtener los detalles del vídeo.');
      }
    } catch (error) {
...
export default {
  ...
  run: async (client, m, args, command, text) => {
    try {
      ...
      const mensaje = `ㅤㅤ 〔ㅤㅤㅤㅤㅤ *⬚⬚* ㅤㅤㅤㅤㅤ〕
ㅤㅤ𖩇ㅤㅤㅤㅤ *𝖸𝖮𝖴𝖳𝖴𝖡𝖤* ㅤㅤㅤㅤ𖩉

ㅤ ┝ㅤ Autor: ${tittle}
ㅤ ┝ㅤ Publicado: ${ago}
ㅤ ┝ㅤ Duración: ${timestamp}
ㅤ ┝ㅤ Visitas: ${vistasTexto}
...
ㅤㅤ ￩ ㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤ ￫`;
      ...
      if (['play', 'yta', 'mp3', 'ytmp3', 'playaudio'].includes(command)) {
        ...
        try {
          ...
          const api = await safeFetch(`${api.url2}/dl/ytmp3?url=${url}&key=${api.key2}`).then(r => r.json());
        ...
        for (let fuente of fuentes) {
          try {
            const res = await safeFetch((fuente.url).then(r => r.json());
            const dl = res?.data?.dl || res?.result?.url || res?.data?.dl || res?.result?.download?.url || res?.downloads?.url || res?.data?.download?.url;
            if (dl) {
              const objeto = { [docMode ? 'document' : 'video']: { url: dl }, fileName: `${title}.mp4`, mimetype: 'video/mp4', caption: `✔️ ${docMode ? "Documento" : "Video"} ${dev}`, thumbnail: thumb };
        ...
        return m.reply("✖️ No se encontró un enlace de descarga válido en ninguna fuente.");
      } else {
        return m.reply("Comando no reconocido.");
      }
    } catch (error) {
      console.error("Error:", error);
      return m.reply(`✖️ *Error:* ${error}`);
    }
  }
};
