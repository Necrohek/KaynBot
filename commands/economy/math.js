...
async function run(client, m, args, command, text, prefix) {
  ...
  if (command === 'responder') {
    if (!juego?.juegoActivo)
      return // client.reply(chatId, '✖️ No hay ningún juego activo. Usa *math <dificultad>* para comenzar uno.', m);
    ...
    if (!primaryBotId || primaryBotId === botId) {
      if (respuestaUsuario === respuestaCorrecta) {
        ...
        return client.reply(chatId, `✔️ Respuesta correcta.\n> *Ganaste ›* ${expaleatorio} Exp`, m);
      } else {
        juego.intentos += 1;
        if (juego.intentos >= 3) {
          clearTimeout(juego.tiempoLimite);
          delete global.math[chatId];
          return client.reply(chatId, '✖️ Te quedaste sin intentos. Suerte a la próxima.', m);
        } else {
          const intentosRestantes = 3 - juego.intentos;
          return client.reply(chatId, `✖️ Respuesta incorrecta, te quedan ${intentosRestantes} intentos.`, m);
        }
      }
    }
    return;
    ...
    globalThis.math[chatId] = {
      ...
      tiempoLimite: setTimeout(() => {
        if (global.math[chatId]?.juegoActivo) {
          ...
          client.reply(chatId, '✖️ Tiempo agotado. El juego ha terminado.', m);
        }
      }, 60000)
    };
  }
...
export default {
  command: ['math', 'matemáticas', 'responder'],
  category: 'rpg',
  run
};
