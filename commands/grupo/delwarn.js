...
export default {
  ...
  run: async (client, m, args) => {
    ...
    if (!user) return m.reply('✖️ No se encontró al usuario en la base de datos.')
    ...
    if (rawIndex?.toLowerCase() === 'all') {
      ...
      return client.reply(
        m.chat,
        `✔️ Se han eliminado todas las advertencias del usuario @${targetId.split('@')[0]} (${name}).`,
        m,
        { mentions: [targetId] }
      )
    }
    ...
    await client.reply(
      m.chat,
      `✔️ Se ha eliminado la advertencia #${index} del usuario @${targetId.split('@')[0]} (${name}).`,
      m,
      { mentions: [targetId] }
    )
  },
