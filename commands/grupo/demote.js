  ...
  run: async (client, m) => {
    ...
    try {
      ...
      if (who === groupMetadata.owner)
        return m.reply('✖️ No puedes degradar al creador del grupo de administrador.')

      if (who === client.user.jid)
        return m.reply('✖️ No puedes degradar al bot de administrador.')

      await client.groupParticipantsUpdate(m.chat, [who], 'demote')
      await client.reply(
        m.chat,
        `✔️ *@${who.split('@')[0]}* ha sido degradado de administrador del grupo.`,
        m,
        { mentions: [who] }
      )
    } catch {
