    ...
    try {
      ...
      if (total >= warnLimit && expulsar) {
        try {
          ...
          message += `\n\n> ㅤ࡛✦֗ El usuario ha alcanzado el límite de advertencias y fue expulsado del grupo.`
        } catch {
          message += `\n\n> ㅤ࡛✦֗ El usuario alcanzó el límite, pero no se pudo expulsar automáticamente.`
        }
      } else if (total >= warnLimit && !expulsar) {
        message += `\n\n> ㅤ࡛✦֗ El usuario ha alcanzado el límite de advertencias.`
      }
