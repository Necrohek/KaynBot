  ...
  run: async (client, m, args) => {
    ...
    let teks2 = armar
      .map((v) => {
        switch (v.type) {
          case 'video':
            return ` 𖣣ֶㅤ֯ 𐩰ㅤ    *Título ›* *${v.title}* 

> 𖣣ֶㅤ֯ 𐩰ㅤ   *Duración ›* ${v.timestamp}
> 𖣣ֶㅤ֯ 𐩰ㅤ   *Subido ›* ${v.ago}
> 𖣣ֶㅤ֯ 𐩰ㅤ   *Vistas ›* ${v.views}
> 𖣣ֶㅤ֯ 𐩰ㅤ   *Url ›* ${v.url}
`.trim()
          case 'channel':
            return `
> ✦֗ Canal › *${v.name}*
> ✦֗ Url › ${v.url}
> ✦֗ Subscriptores › ${v.subCountLabel} (${v.subCount})
> ✦֗ Videos totales › ${v.videoCount}
`.trim()
        }
      })
      .filter((v) => v)
