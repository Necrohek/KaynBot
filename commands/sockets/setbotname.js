export default {
  ...
  run: async (client, m, args, command, text, prefix) => {
    ...
    if (/\s/.test(short)) return m.reply('✖️ El nombre corto no puede contener espacios.')
    config.namebot2 = short.trim()
    config.namebot = long.trim()
    return m.reply(`✔️ El nombre del bot ha sido actualizado!\n\n𓇚ㅤㅤׂ Nombre corto: *${short.trim()}*\n𓇚ㅤㅤׂ Nombre largo: *${long.trim()}*`)
  },
};
