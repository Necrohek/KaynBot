...
const to = new Date().toLocaleDateString('es-CO', { timeZone: 'America/Mexico', year: 'numeric', month: '2-digit', day: '2-digit' }).split('/').reverse().join('-') 
...
console.log(`ㅤ✦֗✦֗ㅤㅤㅤㅤ࣭ㅤㅤㅤㅤ࣭ㅤㅤㅤㅤ࡛✦֗✦֗
ᆺ༑  ${chalk.cyan('𝗕𝗼𝘁   ⦊')} ${chalk.bgMagenta(chalk.white.italic(client.user.id))}
ᆺ༑  ${chalk.cyan('𝗛𝗼𝗿𝗮𝗿𝗶𝗼   ⦊')} ${chalk.black.bgWhite(moment().format('DD/MM/YY HH:mm:ss'))}
ᆺ༑ ${chalk.magentaBright('°o.OO.o°°o.OO.o°°o.OO.o°')}
ᆺ༑  ${chalk.green('𝗨𝘀𝘂𝗮𝗿𝗶𝗼   ⦊')} ${chalk.white(pushname)}
ᆺ༑  ${chalk.green('𝗚𝗿𝘂𝗽𝗼   ⦊')} ${chalk.cyan(m.isGroup ? groupName : 'Chat Privado')}
ᆺ༑  ${chalk.green('𝗜𝗗   ⦊')} ${chalk.cyan(m.isGroup ? from : 'Chat Privado')}
ㅤ✦֗✦֗ㅤㅤㅤㅤ࣭ㅤㅤㅤㅤ࣭ㅤㅤㅤㅤ࡛✦֗✦֗`)  
}
const prefixxy = ['/', '#', '!', '-', '+', '.']
...
// user.stats[today].msgs++
    ...
    if (cmdData.isOwner && !global.owner.map(num => num + '@s.whatsapp.net').includes(sender)) return m.reply(`✖️ El comando *${command}* no existe.\n🇪🇪 Usa *${prefix}help* para ver la lista de comandos disponibles.`)
    if (cmdData.isModeration && !global.mods.map(num => num + '@s.whatsapp.net').includes(sender)) return m.reply(`✖️ El comando *${command}* no existe.\n🇪🇪 Usa *${prefix}help* para ver la lista de comandos disponibles.`)
