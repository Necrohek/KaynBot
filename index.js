...
const displayLoadingMessage = () => {
  console.log(chalk.bold.redBright(`\n\nPor favor, Ingrese el número de WhatsApp.\n` +
      `${chalk.bold.yellowBright("Ejemplo: +52331******")}\n` +
...
┊ ${chalk.blueBright('𖧷𖧷ִㅤㅤㅤ࣭ㅤㅤㅤ࣭ㅤㅤㅤ࡛𖧷𖧷')}
┊ ${chalk.blueBright('┊')} ${chalk.blue.bgBlue.bold.cyan('MÉTODO DE VINCULACIÓN')}
┊ ${chalk.blueBright('𖧷𖧷ִㅤㅤㅤ࣭ㅤㅤㅤ࣭ㅤㅤㅤ࡛𖧷𖧷')}   
┊ ${chalk.blueBright('𖡔ㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤ𖡔')}     
...
async function startBot() {
  ...
  client.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      const reconnect = async (msg, time = 3000) => {
        log.warning(msg + ` (reintentando en ${time / 1000}s)`)
        await delay(time)
        isRestarting = false
        startBot()
      }
      ...
      switch (reason) {
        ...
        case DisconnectReason.connectionClosed:
          await reconnect("Conexión perdida/cerrada")
          break
        case DisconnectReason.restartRequired:
          await reconnect("Reinicio requerido", 2000)
          break
        case DisconnectReason.timedOut:
          await reconnect("Timeout de conexión", 5000)
          break
        ...
        case DisconnectReason.multideviceMismatch:
          ...
          break
        default:
          await reconnect("Error desconocido", 4000)
      }
    }
    ...
    if (connection === "open") {
      ...
      if (!client._keepAlive) {
        client._keepAlive = setInterval(() => {
          try { client.sendPresenceUpdate("available") } catch {}
        }, 20000)
      }
      ...
      if (!client._watchdog) {
        client._watchdog = setInterval(() => {
          ...
          if (diff > 120000) {
            log.warning("Watchdog: congelamiento detectado, reiniciando...")
