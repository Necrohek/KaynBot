...
async function getFetchBuffer(url) {
...
client.getFile = async (PATH, saveToFile = false) => {
    ...
    : PATH instanceof ArrayBuffer
      ...
      : /^data:.*;base64,/i.test(String(PATH))
        ...
        : /^https?:\/\//i.test(String(PATH))
          ? (res = Buffer.from(await getFetchBuffer(String(PATH))))
  ...
  if (data && saveToFile && !filename) {
    const tmpDir = path.join(process.cwd(), 'lib', 'system', 'tmp');
...
const canalSeleccionado = canales[Math.floor(Math.random() * canales.length)]
  ...
  client.sendContextInfoIndex = async (
    ...
    const namebot2 = config.body || dev
